...
```
// ✅ 良い例: 役割が明確
userAuthentication = new UserAuthenticationService()
...
```
テスト: TaskService - タスク作成

ケース1: 正常なデータの場合、タスクを作成できる
  // Given: 準備
...
ケース2: タイトルが空の場合、ValidationError を返す
  // Given: 準備
  service = new TaskService(mockRepository)
  invalidData = { title: "" }

  // When/Then: 実行と検証
  assert service.create(invalidData) throws ValidationError
```

具体的なテストフレームワークの書き方は、実装ガイド (implementation.md) を参照してください。
...
| 対象 | カバレッジ目標 | 理由 |
|---|---|---|
| 全体 (ブランチ/関数/行) | 80% 以上 | 基本的な品質水準 |
| ビジネスロジック層 | 90% 以上 | 最も重要な層であるため高い基準を設定 |
| プレゼンテーション層 | 70% 以上 | I/O 境界のため網羅が難しい箇所がある |

**理由**:
- 重要なビジネスロジック層は高いカバレッジを要求
- I/O 境界に近い層は低めでも許容
- 100% を目指さない (コストと効果のバランス)

カバレッジ計測ツールの設定は、実装ガイド (implementation.md) を参照してください。
...
```
taskMap = buildMap(tasks, key=t.id)
result  = ids.map(id => taskMap.get(id))
```
```
...
以下のカテゴリの自動化を導入する。具体的なツールと設定は言語・フレームワークに応じて選定する。

| カテゴリ | 目的 | 例 |
|---|---|---|
| Lint | コーディング規約の自動検出・潜在バグの防止 | ESLint, staticcheck, golangci-lint |
| フォーマット | コードスタイルの自動整形 | Prettier, gofmt, goimports |
| 型チェック / 静的解析 | 型安全性・論理エラーの事前検出 | tsc, go vet, mypy |
| テスト実行 | ユニット・統合テストの自動実行とカバレッジ計測 | Vitest, go test, pytest |
| ビルド確認 | コンパイル・ビルドが成功することの検証 | tsc, go build, cargo build |

**実装方法**:

**1. CI/CD パイプライン**

push・PR 作成時に以下を自動実行する:
...
CI/CD プラットフォーム (GitHub Actions, GitLab CI 等) 上で設定する。具体的なワークフロー定義は実装ガイド (implementation.md) を参照。

**2. Pre-commit フック**

コミット前にローカルで品質チェックを実行する仕組みを導入する:
- ステージングされたファイルに対して Lint + フォーマットを自動実行
- 必要に応じて型チェック・テストも実行

ツール例: Husky + lint-staged, pre-commit (Python), lefthook
